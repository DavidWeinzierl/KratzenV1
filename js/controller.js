...
export let areOtherPlayersCardsHidden = false; // NEW: State for card visibility
...
// NEW: Function to toggle card visibility for other players
export function toggleOtherPlayersCardVisibility() {
    ...
    if (!isSimulationRunning) {
        ...
        if (gameState) {
            renderGame(gameState); // Re-render to reflect the change
        }
    }
}
    ...
    if (stage === GAME_PHASE.BIDDING_STAGE_1) {
        ...
        if (currentGameState.oderPlayer) { // An Oder was the initial declaration from Stage 1
            ...
            if (odererIsStillTheDeclarer.status !== PLAYER_STATUS.FOLDED && (activePartners.length > 0 || odererPlaysAloneByChoice)) {
                ...
                if (!isSimulationRunning) {
                    if (odererIsStillTheDeclarer.status === PLAYER_STATUS.FOLDED) {
...
export async function runBatchSimulation(numGames, progressCallback) {
    ...
    for (let i = 0; i < numGames; i++) {
        ...
        if ((i + 1) % Math.max(1, Math.floor(numGames / 20)) === 0 || i + 1 === numGames) {
             if(!isSimulationRunning) logMessage(`SIM: Completed ${i + 1} / ${numGames} games...`);
             else if ( (i+1) === numGames) logMessage(`SIM: Completed ${i + 1} / ${numGames} games...`);
        }
    }
